...
DECLARE
    ...
    _is_broadcasted BOOLEAN;
BEGIN
    -- ensure that we are not broadcasting the same transaction twice in the db
    SELECT is_broadcasted INTO _is_broadcasted FROM unsigned_transactions WHERE id = _unsigned_tx_id;
    IF _is_broadcasted THEN
        RAISE EXCEPTION 'Transaction already broadcasted';
    END IF;
    ...
    -- Update the unsigned transaction to be broadcasted
